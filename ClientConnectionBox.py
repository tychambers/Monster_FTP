...
def client_connect_box(ftpclient, sftpclient):
    ...
    def choose_connection(self):
        ...
        if self == "FTP":
            ...
            def ftp_connect():
                ...
                # messagebox.showinfo("Connection Status", connection_code)
                if connection_code == "Success":
                    ftpclient.being_used = True
                    root.withdraw()
                    root.quit()
                else:
                    pass
            ...
            def sftp_connect():
                ftpclient.being_used = False
                client = sftpclient

                host = sftp_host_entry.get()
                port = int(sftp_port_entry.get())
                username = sftp_un_entry.get()
                password = sftp_pw_entry.get()
                pk_path = sftpclient.pk_path
                pk_pw = sftpclient.pk_password
                ...
                if pk_path == "":
                    ...
                    connection_code = client.connect_with_key(host, port, username, password, pk_path, pk_pw)
                    pop_up(connection_code)
                    # messagebox.showinfo("Connection Status", connection_code)
                if connection_code == "Success":
                    sftpclient.being_used = True
                    root.withdraw()
                    root.quit()
                else:
                    pass
    ...
    custom_dropdown = ctk.CTkOptionMenu(root, variable=connection_type_dropdown, values=connection_types,
                                        bg_color="black",
